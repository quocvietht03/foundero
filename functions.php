<?php
/* Register Sidebar */
if (!function_exists('foundero_register_sidebar')) {
	function foundero_register_sidebar()
	{
		register_sidebar(array(
			'name' => esc_html__('Main Sidebar', 'foundero'),
			'id' => 'main-sidebar',
	...
	add_action('widgets_init', 'foundero_register_sidebar');
}

/* Get icon SVG HTML */
function foundero_get_icon_svg_html( $icon_file_name ) {
...
/* Enqueue Script */
if (!function_exists('foundero_enqueue_scripts')) {
	function foundero_enqueue_scripts()
	{
		wp_enqueue_style('foundero-fonts', get_template_directory_uri() . '/assets/css/fonts.css',  array(), false);
		...
		if (function_exists('get_field')) {
			...
			if ($dev_mode && !empty($custom_style)) {
				wp_add_inline_style('foundero-style', $custom_style);
			}
			...
			if ($dev_mode && !empty($custom_script)) {
				wp_add_inline_script('foundero-main', $custom_script);
			}
		}
		...
		wp_localize_script('foundero-main', 'AJ_Options', $js_options);

		wp_enqueue_script('foundero-main');
	}
	add_action('wp_enqueue_scripts', 'foundero_enqueue_scripts');
}
/* Add Stylesheet And Script Backend */
if (!function_exists('foundero_enqueue_admin_scripts')) {
	function foundero_enqueue_admin_scripts($hook)
	{
		wp_enqueue_style('foundero-fonts', get_template_directory_uri() . '/assets/css/fonts.css',  array(), false);
		wp_enqueue_script('foundero-admin-main', get_template_directory_uri() . '/assets/js/admin-main.js', array('jquery'), '', true);
		wp_enqueue_style('foundero-admin-main', get_template_directory_uri() . '/assets/css/admin-main.css', array(), false);

	}
	add_action('admin_enqueue_scripts', 'foundero_enqueue_admin_scripts');
}
